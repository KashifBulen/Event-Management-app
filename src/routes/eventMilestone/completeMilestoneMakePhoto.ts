...
const handler: Handler<"completeMilestoneMakePhoto"> = async (request, { user }) => {
  //   const userId = user.userId;
  const eventId = new mongoose.Types.ObjectId(request?.eventId);
  const { milestoneId, photo } = request;
  if (!eventId) {
    throw new Boom(
    ...
    if (eventMilestons) {
      for (const milestone of eventMilestons) {
        if (milestone.milestoneIdentifier === Status.MILESTONE_MAKE_PHOTO) {
          ...
          if (eventMilestoneInfo) {
            ...
            if (!milestoneExists) {
              ...
              await EventJoin.updateOne(
                ...
                {
                  $set: {
                    'extraInfo.milestone_make_photo_data': {
                      milestone: Status.MILESTONE_MAKE_PHOTO,
                      photoUrl: photo,
              ...
              // eventMilestoneInfo.extraInfo.milestone_make_photo_data = {
              //   milestone:Status.MILESTONE_MAKE_PHOTO,
              //   photoUrl:photo
              // };
