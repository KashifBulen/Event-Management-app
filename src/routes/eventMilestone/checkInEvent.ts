...
const handler: Handler<"checkInEvent"> = async (request, { user }) => {
  ...
  if (!eventId) {
    ...
    if (eventMilestons) {
      for (const milestone of eventMilestons) {
        if (milestone.milestoneIdentifier === Status.MILESTONE_CHECK_IN) {
    ...
    return { message: "User checkedin successfully" };
  }
};
