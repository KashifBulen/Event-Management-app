...
const { NOT_FOUND, FORBIDDEN, UNAUTHORIZED, BAD_REQUEST, OK, REQUEST_TIMEOUT, INTERNAL_SERVER_ERROR } = https;
...
function errorHandlingWrap<O extends Operation>(
  ...
  return async (e, req, res) => {
    ...
    try {
      ...
      // if auth failed
      if (e.security && !e.security?.authorized && "firebaseAuth" in e.security) {
        throw new Boom(`${e.security.firebaseAuth.error}`, { statusCode: UNAUTHORIZED })
      }

      const user: TokenProps | undefined = e.security?.firebaseAuth;
...
const makeApi = (
  ...
  api.registerSecurityHandler("firebaseAuth", async (e) => {
    try {
      ...
      // diable it for now
      // //@ts-ignore
      // const user = await firebase.auth().verifyIdToken(decoded?.idToken as string);
      // if (!user) {
      //   throw new Boom("Insufficient Access", { statusCode: FORBIDDEN });
      // }

      // console.log('userssssss', decoded)
